...
const double tau_kur  = 0.1;         // tau in the Kuramoto equation
const double r_kur    = 2*r_int; // interaction range = 2r

const double alpha_new = 1;
...
struct Particle {
    ...
    double omega;   // <-- ADD THIS (chirality)
};
// ================= PBC =======================
...
//     double omega0 = 2.0;   // chirality strength
...
//         // ---- CHIRALITY (ONLY ONE PARTICLE) ----
//         if(N == 2){
//             if(i == 0)
//                 p.omega = omega0;   // first particle is chiral
//             else
//                 p.omega = 0.0;      // second is non-chiral
//         } else {
//             p.omega = 0.0;          // default for larger N
//         }
...
//     int max_k = ceil(sqrt(N));  // rough outer ring index
...
//         if(i == 0)      P[i].theta_n = M_PI/2;
//         else            P[i].theta_n = M_PI/2;
    ...
    for(int i = 0; i < n; i++){
        for(int j : neighbors[i]){
            ...
            if(alpha_CF != 0.0){
                ...
                double wi    = 0.25 * (1.0 + dot_j)*shared;
                JCF[i].first  += wi * dxn;
                JCF[i].second += wi * dyn;

                double dot_i = dxn * P[i].cos_theta + dyn * P[i].sin_theta;
                double wj    = 0.25 * (1.0 - dot_i)*shared;
                JCF[j].first  -= wj * dxn;
                JCF[j].second -= wj * dyn;
            }
        }
...
double dtheta = dt * P[i].omega   // <-- ADD THIS LINE
              + dt * alpha_CF * (J0[i].first  * (-P[i].sin_theta)
    ...
    for(int i = 0; i < n; i++){
        ...
        double d0 = P[i].omega   // <-- ADD
                + alpha_CF * (J0[i].first  * (-P[i].sin_theta)
                            + J0[i].second *   P[i].cos_theta)
                + (1.0 * tau_kur) * JK0[i]
                + alpha_new       * (JN0[i].first  * (-P[i].sin_theta)
                                    + JN0[i].second *   P[i].cos_theta);

        double d1 = Ppred[i].omega   // <-- ADD
                + alpha_CF * (J1[i].first  * (-Ppred[i].sin_theta)
