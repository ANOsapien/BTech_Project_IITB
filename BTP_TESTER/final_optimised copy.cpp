...
const double tau_kur  = 0.0;         // tau in the Kuramoto equation
const double r_kur    = r_int; // interaction range = 2r

// New asymmetric torque: weight (1+ni.xij)(1-nj.xij)/4, acts at 2r
...
void compute_forces(const vector<Particle>& P,
                    ...
                    vector<double>& JKur,
                    vector<double>& JNew){      // <-- new asymmetric torque accumulator
    ...
    // --- conservative force + non-reciprocal torque within r_int ---
    for(int i = 0; i < n; i++){
...
//     double suppress = 0.5 * (1.0 - dot_ninj);   // → 0 when parallel, 1 when anti-parallel

//     double dot_j = dxn * P[j].cos_theta + dyn * P[j].sin_theta;
//     double wi    = 0.5 * (1.0 + dot_j) * suppress;
//     JCF[i].first  += wi * dxn;
//     JCF[i].second += wi * dyn;

//     double dot_i = dxn * P[i].cos_theta + dyn * P[i].sin_theta;
//     double wj    = 0.5 * (1.0 - dot_i) * suppress;
//     JCF[j].first  -= wj * dxn;
//     JCF[j].second -= wj * dyn;
// }
    }

    // --- Kuramoto torque + new asymmetric torque, both within 2*r_int ---
    for(int i = 0; i < n; i++){
        for(int j : neighbors_kur[i]){
            ...
            // Kuramoto: sin(theta_j - theta_i)
            double s = sin(P[j].theta_n - P[i].theta_n);
            JKur[i] += s;
            JKur[j] -= s;

            // New asymmetric torque
            // weight on i: (1 + ni.xij)(1 - nj.xij) / 4
            // weight on j: (1 + nj.xji)(1 - ni.xji) / 4
            //            = (1 - nj.xij)(1 + ni.xij) / 4  — same value, non-reciprocal in direction
            // torque direction: x̂_ij rotated -90° = (dyn, -dxn)
            // non-reciprocal: j uses x̂_ji = -x̂_ij, so its perp = (-dyn, dxn)
            if(alpha_new != 0.0){
                double dot_i    = dxn * P[i].cos_theta + dyn * P[i].sin_theta;   // n_i . x_ij
                double dot_j    = dxn * P[j].cos_theta + dyn * P[j].sin_theta;   // n_j . x_ij
                double dot_ninj = P[i].cos_theta * P[j].cos_theta
                                + P[i].sin_theta * P[j].sin_theta;               // n_i . n_j (symmetric)
 
                double shared = 1.0 + dot_ninj;   // common second factor
 
                // torque on i: (1 - n_i.x_ij)(1 + n_i.n_j)/4, direction x_ij rotated -90
                double perp_i = dyn * (-P[i].sin_theta) + (-dxn) * P[i].cos_theta;
                double wi     = 0.25 * (1.0 - dot_i) * shared;
                JNew[i]      += wi * perp_i;
 
                // torque on j: x_ji = -x_ij so first factor flips to (1 - dot_j), perp flips too
                double perp_j = (-dyn) * (-P[j].sin_theta) + (dxn) * P[j].cos_theta;
...
void update(vector<Particle>& P, mt19937 &rng){
    ...
    static vector<double> JN0, JN1;           // <-- new asymmetric torque buffers
